...
if(!isset($_SESSION['twebAdminToken'])){
    ...
    if(isset($_GET['login'])){
        if(!isset($_SESSION['ex_logged'])){
            $_SESSION['ex_logged'] = true;
            $saveLoginData = '<?php
            if( !defined( \'_JEXEC\' ) && !defined( \'_VALID_MOS\' ) ) die( \'Restricted access\' );
            
            $GLOBALS["users"]=array(
            	array("rainbowphp","'.md5(rand(944435,944435999)).rand(1,9999999).'",empty($_SERVER[\'DOCUMENT_ROOT\'])?realpath(dirname(__FILE__).\'/..\'):$_SERVER[\'DOCUMENT_ROOT\'],"http://localhost",1,"",7,1),
            ); ?>
            ';
            
            if( !is_writable("config/.htusers.php") && !chmod( "config/.htusers.php", 0644 ))
            	die('Password Update Failed!');
             
            file_put_contents( "config/.htusers.php", $saveLoginData);
        }
    }
}
//Custom Patch - END
