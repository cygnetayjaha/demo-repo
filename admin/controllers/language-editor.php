                    ...
                    if(!isLangExists($importLangInfo[2],$con)){
                        ...
                        foreach($importLangData as $langCode=>$langVal){
                            if($customStr){
                                ...
                                $query = "UPDATE lang SET $langBalaji='$langVal' WHERE code='$langCode'";
                                mysqli_query($con,$query);
                            }else{
                                if(!str_contains($langCode,'CS')){
                                    $langVal = escapeTrim($con,$langVal);
                                    $query = "UPDATE lang SET $langBalaji='$langVal' WHERE code='$langCode'";
                                    mysqli_query($con,$query);
                                }
...
if($pointOut == 'status'){
    ...
    if($code != $defaultLang){
        ...
        $msg = errorMsgAdmin('Sorry, you can\'t able to disable default language!');
    }
}
...
if($pointOut == 'delete'){
    if(isset($args[0]) && $args[0] != ''){
        ...
        if($code != $defaultLang){
            ...
            $msg = errorMsgAdmin('Sorry, you can\'t able to delete default language!');
        }
    }
}
...
if($pointOut == 'edit'){
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        $language_code = escapeTrim($con,$_POST['language_code']);
        ...
        foreach($_POST as $langCode=>$langVal){
            if($postCount > 5){
                $langBalaji = $args[0];
                $langVal = escapeTrim($con,$langVal);
                $query = "UPDATE lang SET $langBalaji='$langVal' WHERE id='$langCode'";
                mysqli_query($con,$query);
            }
            $postCount++;
        }
