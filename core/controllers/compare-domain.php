...
defined('DB_DOMAIN') or die(header('HTTP/1.0 403 Forbidden'));
...
foreach($outArr as $outKey){
    if(str_contains($metatitle, $outKey[0], true)){
    ...
    if(str_contains($metadescription, $outKey[0], true)){
...
if(str_contains($dataFirst1,'301')){
    $re301 = true;
    $resolveClass = 'passedBox';
    $resolveMsg = $lang['AN46'];
}

if(str_contains($dataFirst2,'301')){
    $re301= true;
    $resolveClass = 'passedBox';
    $resolveMsg = $lang['AN46'];
}
...
$facebook_like = number_format(Trim($social_count[0]));
$linkedin_count = number_format(Trim($social_count[1]));
$gplus_count = number_format(Trim($social_count[2]));
$stumble_count = number_format(Trim($social_count[3]));
...
<div class="msgBox">   
    ...
    <br />
    <div class="altImgGroup">
        <br><div class="social-box"><i class="fa fa-facebook social-facebook"></i> Facebook Likes: '.$facebook_like.'</div><br>
        <div class="social-box"><i class="fa fa-google-plus social-google"></i> PlusOne: '.$gplus_count.'</div><br>
        <div class="social-box"><i class="fa fa-stumbleupon social-stumbleupon"></i> StumbleUpon: '.$stumble_count.'</div><br>
        <div class="social-box"><i class="fa fa-linkedin social-linkedin"></i> LinkedIn: '.$linkedin_count.' </div>
    </div>
    <br />
</div>
...
foreach ($list[1] as $badWord){
    if(str_contains($firstTitle, trim($badWord), true))
        $badWordBan = true;
    if(str_contains($secTitle, trim($badWord), true))
        $badWordBan = true;
}
if(!$badWordBan){
    foreach ($list[2] as $badWord){
        if(str_contains($firstDes, trim($badWord), true))
            $badWordBan = true;
        if(str_contains($secDes, trim($badWord), true))
            $badWordBan = true;
    }
}
if(!$badWordBan){
    foreach ($list[3] as $badWord){
        if(str_contains($firstKey, trim($badWord), true))
            $badWordBan = true;
        if(str_contains($secKey, trim($badWord), true))
            $badWordBan = true;
    }
}
if($badWordBan){
...
if(str_contains($dataSec1,'301')){
    $re301 = true;
    $resolveClass = 'passedBox';
    $resolveMsg = $lang['AN46'];
}

if(str_contains($dataSec2,'301')){
    $re301= true;
    $resolveClass = 'passedBox';
    $resolveMsg = $lang['AN46'];
}
