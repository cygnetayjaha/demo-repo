...
function addLang($langData,$con){
    //Language Data -> //Sort Order //Enabled //Language Code //Language Name //Author //Hide&Show //Direction(LTR or RTL)
    $allLangs = array();
...
function langUpdateAll($code, $langArr, $con){
    ...
    foreach($allLangs as $lang){
        if(in_array($code,$lang, true)){
            $newLangs[] = array($langArr[0],$lang[1],$langArr[1],$langArr[2],$lang[4],$langArr[3],$langArr[4]);
        }else
            $newLangs[] = $lang;
    }
    $newLangs = serialize($newLangs);
