...
class whois
{
    private $WHOIS_SERVERS = array(
        "com" => array("whois.verisign-grs.com", "/Creation Date:(.*)/", "/Updated Date:(.*)/", "/Expiration Date:(.*)/"),
        "net" => array("whois.verisign-grs.com", "/Creation Date:(.*)/", "/Updated Date:(.*)/", "/Expiration Date:(.*)/"),
        ...
        "biz" => array("whois.neulevel.biz", "/Domain Registration Date:(.*)/", "/Domain Last Updated Date:(.*)/", "/Domain Expiration Date:(.*)/"),
        ...
        "us" => array("whois.nic.us", "/Domain Registration Date:(.*)/", "/Domain Last Updated Date:(.*)/", "/Domain Expiration Date:(.*)/"),
        ...
        "me" => array("whois.nic.me", "/Domain Create Date:(.*)/", "/Domain Last Updated Date:(.*)/", "/Domain Expiration Date:(.*)/"),
        ...
        "ru" => array("whois.nic.ru", "/created:(.*)/", "/Last update of WHOIS database:(.*)/", "/paid-till:(.*)/"),
    ...
    private function queryWhois($server, $domain) {
        if ($server == "whois.nic.ac")
        {
           $data = $this->getMyData("http://www.nic.ac/cgi-bin/whois?DOMAIN=$domain","http://www.nic.ac");
            $data = explode("Domain Information",$data);
            $data =  explode("Primary Nameserver",$data[1]);
            $out = strip_tags($data[0]);
        }
        elseif ($server == "whois.nic.io")
        {
           $data = $this->getMyData("http://www.nic.io/cgi-bin/whois?DOMAIN=$domain","http://www.nic.io");
            $data = explode("Domain Information",$data);
            $data =  explode("Primary Nameserver",$data[1]);
            $out = strip_tags($data[0]);
        }
        elseif ($server == "whois.nic.sh")
        {
           $data = $this->getMyData("http://www.nic.sh/cgi-bin/whois?DOMAIN=$domain","http://www.nic.sh");
            $data = explode("Domain Information",$data);
            $data =  explode("Primary Nameserver",$data[1]);
            $out = strip_tags($data[0]);
        } else {
        ...
        if($serverIP != ''){
            $fp = @fsockopen($serverIP, 43, $errno, $errstr, 20) or $out = "Socket Error " . $errno .
                " - " . $errstr;
